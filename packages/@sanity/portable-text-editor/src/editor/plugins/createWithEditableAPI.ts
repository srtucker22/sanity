          ...
          try {
            const [node] = Array.from(
              Editor.nodes(editor, {
                mode: 'lowest',
                at: editor.selection.focus,
                match: (n) => SlateElement.isElement(n) && n._type !== undefined,
                voids: true,
              })
            )[0] || [undefined]
      ...
      activeAnnotations: (): PortableTextBlock[] => {
        ...
        try {
          ...
          for (const [span, path] of spans) {
            ...
            if (block && SlateElement.isElement(block) && Array.isArray(block.markDefs)) {
                  ...
                  if (editor.selection && Text.isText(textNode)) {
                    Transforms.setNodes(
                      ...
                      },
                      {
                        at: editor.selection,
                        match: (n) =>
                          Text.isText(n) && n._type === portableTextFeatures.types.span.name,
                      }
                    )
                  }
                })
          ...
          Editor.withoutNormalizing(editor, () => {
            if (selection && Range.isExpanded(selection)) {
              ...
              spans.forEach(([span, path]) => {
                const [block] = Editor.node(editor, path, {depth: 1})
                if (SlateElement.isElement(block) && Array.isArray(block.markDefs)) {
                  block.markDefs

...
function annotationStyle(props: AnnotationStyleProps) {
  ...
  const bgKey = $toneKey === 'primary' ? 'bg' : 'bg2'

  return css`
    text-decoration: none;
    display: inline;
    background: ${theme.sanity.color.selectable[$toneKey].enabled[bgKey]};
    border-bottom: 2px dotted ${theme.sanity.color.selectable[$toneKey].enabled.fg};
    color: ${theme.sanity.color.selectable[$toneKey].enabled.fg};

    @media (hover: hover) {
      &:hover {
        background: ${theme.sanity.color.selectable[$toneKey].hovered.bg2};
